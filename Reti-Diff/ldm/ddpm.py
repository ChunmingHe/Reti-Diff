...
class DDPM(nn.Module):
    ...
    def p_sample(self, x, t, c,clip_denoised=True, repeat_noise=False):
        ...
        #return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise, predicted_noise
    ...
    def forward(self, img,x=None):
        ...
        if self.training:
            ...
            for i in reversed(range(0, self.num_timesteps)):
                deg_prep, predicted_noise = self.p_sample(deg_prep, torch.full((b,), i,  device=device, dtype=torch.long), c,
                                clip_denoised=self.clip_denoised)
            ...
            deg_prep = x_noisy
            for i in reversed(range(0, self.num_timesteps)):
                deg_prep, _ = self.p_sample(deg_prep, torch.full((b,), i,  device=device, dtype=torch.long), c,
                                clip_denoised=self.clip_denoised)
            return deg_prep
